new (class {
  ...
  addGroup(t, s) {
    ...
    for (let s of ((this.gMaps[t] = {}), this.groups))
      for (let t of this.groups)
        this.gMaps[s.color][t.color] ??=
          this.random(this.minRandomG, this.maxRandomG) / 10;
  }
  addAtoms(t, s) {
  ...
  randomGMaps() {
    for (let t in this.gMaps) {
      let s = this.gMaps[t];
      for (let t in s)
        s[t] = this.random(this.minRandomG, this.maxRandomG) / 10;
    }
  ...
  randomAtomsXY() {
    for (let t = 0; t < this.groups.length; ++t) {
      ...
      for (let t = 0; t < i.length; ++t) {
        let s = i[t];
        (s.x = this.random(1, this.width - 1)),
          (s.y = this.random(1, this.height - 1));
      }
    }
  }
  clearScreen() {
  ...
  rule(t, s) {
    ...
    for (let t = 0; t < h.length; ++t) {
      ...
      (o.vx = (o.vx + e) * 0.5),
        ...
        o.x < 0
          ? (o.x += this.width)
          : o.x >= this.width && (o.x -= this.width),
        o.y < 0
          ? (o.y += this.height)
          : o.y >= this.height && (o.y -= this.height);
    }
  }
  render() {
